...
\begin{center}
  ...
  {\large Axel Roudaut --- Ingénieur DevSecOps --- Platform \& Data}\\[4pt]
...
\textbf{Ingénieur DevSecOps avec 7+ ans d'expérience} dans la conception et l'exploitation d'infrastructures distribuées en environnements air-gap renforcés (Thales, Ministère des Armées). Spécialisé en CI/CD, plateformes Kubernetes, automatisation VMware, supervision, pipelines de traitement Kafka et indexation Elasticsearch à grande échelle, ainsi que développement d'outillage en Python ; montée en compétences sur l'infrastructure IA et l'inférence LLM. Ouvert à des postes DevOps, SRE, Platform ou Data Engineering à \textbf{Montpellier ou en full remote} dès \textbf{octobre 2026}.
...
\begin{itemize}
  \item \textbf{Intitulé du poste} : Platform Engineer --- Produits de cybersécurité
...
\begin{itemize}
  \item \textbf{Intitulé du poste} : Data \& DevOps Engineer --- Supervision de sécurité (SIEM)
...
\begin{itemize}
  ...
  \item Alerting sur indicateurs avancés, étage par étage : profondeur des files persistantes Logstash, décalage (\emph{lag}) des consommateurs Kafka, entrées/sorties réseau des nœuds ksqlDB, IO et espace disque sur Elasticsearch --- de quoi intervenir avant la saturation plutôt qu'après la perte.
  \item Validation de bout en bout par événements de synthèse (\emph{canaries}) : un journal calibré pour déclencher une règle connue est injecté à la source, puis contrôlé en sortie (parsé, catégorisé, indexé, corrélé dans la fenêtre attendue). Un composant peut se déclarer sain pendant que la chaîne cesse de produire.
\end{itemize}
...
\subsection{5. Formations professionnelles --- Ministère des Armées (2019 -- 2022)}

\begin{itemize}
  \item \textbf{Cadre} : formation continue, poste de Data \& DevOps Engineer
  \item \textbf{Thèmes} : Elasticsearch, Apache Spark
\end{itemize}

\subsubsection{Contenu}

Formations suivies en appui de la chaîne de supervision de sécurité (SIEM) : indexation et exploitation des données sous Elasticsearch, traitements distribués avec Apache Spark.\stack{Elasticsearch, Apache Spark}

\subsection{6. Autoformation --- Infrastructure IA et inférence LLM (en cours)}

\begin{itemize}
  \item \textbf{Format} : e-learning et veille technique
\end{itemize}

\subsubsection{Contenu}

Prolongement du Platform Engineering et du DevSecOps vers les plateformes d'IA générative, en s'appuyant sur une expérience d'industrialisation, de sécurisation et de supervision d'infrastructures en environnements contraints. Les thèmes travaillés couvrent le cycle de vie d'une plateforme d'inférence :

\begin{itemize}
  \item \textbf{Industrialisation} : déploiement automatisé de serveurs d'inférence LLM sur Kubernetes, allocation et partage des ressources GPU, pipelines CI/CD intégrant l'évaluation des modèles.
  \item \textbf{Architecture et sécurité} : architectures de référence des plateformes d'IA générative, isolation réseau, gouvernance des images et des modèles, risques propres aux LLM (référentiel OWASP).
  \item \textbf{Exploitation} : observabilité des services d'inférence (latence au premier token, débit, occupation du cache KV et des GPU) et alerting associé.
\end{itemize}
\stack{vLLM, Kubernetes, GPU, Inférence LLM, Quantification, Cache KV, OWASP LLM, Observabilité}
